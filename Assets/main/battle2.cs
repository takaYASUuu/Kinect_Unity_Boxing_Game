...
public class battle2 : MonoBehaviour
{
    ...
    public static int damege_state2 = 0;
    public static int guardsuccess_state2 = 0;
    ...
    {
        if ((gameObject.tag == "hand2") && Time.time - start_time > cooltime)
        {
    ...
    void Update()
    {
        if (Time.time - start_time > cooltime && colList.Count >= 1)
        {
            ...
            {
                if (checkObj.tag == "hand")
                {
                    hand_state = 1;
                    guard_state = 0;
                    body_state = 0;
                }
                else if (checkObj.tag == "guard" && hand_state != 1)
                {
                    guard_state = 1;
                    body_state = 0;
                }
                else if (checkObj.tag == "body" && hand_state != 1 && guard_state != 1)
                {
                    body_state = 1;
                }
            }
